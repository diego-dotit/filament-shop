...
function setupAuthStub({
  ...
  const user = isAuthenticated ? ref({ id: 1, name: userName, email: 'alice@example.com' }) : ref(null)
...
describe('Product detail page — review form integration', () => {
  const makeProduct = (overrides: Record<string, unknown> = {}) => ({
    ...
    variants: [{ id: 10, sku: 'SKU-10', price: '29.99', stock_quantity: 5, attributes: { color: 'Red' } }],
    attributes: { material: 'PLA' },
    reviews: [
      { id: 1, rating: 5, comment: 'Excellent!', customer_name: 'Alice' },
    ],
    ...overrides,
  })
