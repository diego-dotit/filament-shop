...
describe('useCheckout composable', () => {
  ...
  it('allows the same address to be selected for both billing and shipping', async () => {
    const { useCheckout } = await import('../composables/useCheckout')
    const { billingAddressId, shippingAddressId, selectBillingAddress, selectShippingAddress } = useCheckout()
  ...
  it('submitOrder: sets orderConfirmation on success with order ID and total', async () => {
    ...
    const { useCheckout } = await import('../composables/useCheckout')
    const { orderConfirmation, selectBillingAddress, selectShippingAddress, submitOrder } = useCheckout()
  ...
  it('isSubmitting: is true during submitOrder and false after completion', async () => {
    ...
    const pendingPromise = new Promise((res) => { resolveOrder = res })
    mockFetch.mockReturnValueOnce(pendingPromise)

    const { useCheckout } = await import('../composables/useCheckout')
    const { isSubmitting, selectBillingAddress, selectShippingAddress, submitOrder } = useCheckout()
