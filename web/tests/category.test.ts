...
const globalStubs = {
  ...
  ProductCard: { template: '<div class="product-card">{{ product.name }}</div>', props: ['product'] },
}
...
describe('Category page [slug].vue', () => {
  ...
  it('renders a back link to the homepage', async () => {
    ...
    const homeLink = links.find(l => l.text().toLowerCase().includes('home') || l.attributes('href') === '/' || l.attributes('to') === '/')
